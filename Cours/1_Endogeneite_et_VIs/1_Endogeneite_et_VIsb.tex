...
\centering
	...
	{\scshape\Large \textsc{Économétrie: S2, UGA, M1 Miash}\par}
	\vspace{1cm}
	%{\scshape\large \textsc{Extremum Estimators(1)}\par}
	%\vspace{1cm}
	{\Large\bfseries \textsc{1: Modèles linéaires} \par}
    \vspace{1cm}   
	{\Large\bfseries \textsc{Endogénéité et variables instrumentales} \par}
	\vspace{1cm}
...
La convergence de $\hat{\beta}^{VI}$ est obtenue sous \eqref{eq9}-\eqref{eq10} auxquelles on doit 
